...
class CGCEngineReal:
    async def trigger_cgc_prefill(self, payload):
        ...
        async def _do_http_gateway_call():
            ...
            req_payload.setdefault("model", str(os.environ.get("CGC_CLOUD_OPENAI_MODEL") or "deepseek-v4-flash"))
            ...
            def _post():
                print(f"[DEBUG] Sending to SGLang: {req_payload}"); return requests.post(gateway_url, json=req_payload, headers=headers, timeout=(10, 1800))
        ...
        if dsml_match:
            ...
            if func_name == "execute_bash":
                ...
                try:
                    ...
                    args_dict = {}
                    param_matches = re.finditer(r'''<[|｜]DSML[|｜]parameter\s+name=["']([^"']+)["'][^>]*>(.*?)</[|｜]DSML[|｜]parameter>''', content, re.DOTALL)
                    for m in param_matches:
                        args_dict[m.group(1)] = m.group(2).strip()
                    if not args_dict:
                        args_dict = {"command": content}
...
async def ollama_tags():
    ...
    return {
        "models": [
            {"name": "deepseek-v4-flash", "model": "deepseek-v4-flash", "modified_at": datetime.now(timezone.utc).isoformat(), "size": 32000000000, "digest": "sha256:cgc_hash_1", "details": {"format": "gguf", "family": "llama", "families": ["llama"], "parameter_size": "32B", "quantization_level": "FP8"}},
