...
def _log_moe_weight_load_shape_mismatch(
    *,
    ...
    logger.error(
        "FusedMoE weight load shape mismatch: "
        "weight_name=%s param_shape=%s loaded_weight_shape=%s shard_id=%s "
        "expert_id=%s quant_method_class=%s quant_config_class=%s "
        "quant_config_is_fp4_experts=%s exc=%r",
        weight_name,
        tuple(param.shape),
        ...
        getattr(quant_config, "is_fp4_experts", None),
        exc,
    )
...
def create_moe_dispatcher(moe_runner_config: MoeRunnerConfig) -> BaseDispatcher:
    ...
    if (
        ...
        return MaybeTboDeepEPDispatcher(
            ...
            async_finish=not disable_deepep_async,
            return_recv_hook=not disable_deepep_async,
        )
        ...
        if "scale" in weight_name or "zero" in weight_name or "offset" in weight_name:
            ...
            if quant_method == FusedMoeWeightScaleSupported.CHANNEL.value:
                ...
                try:
                    ...
                    if "size of tensor a" in str(exc):
                        _log_moe_weight_load_shape_mismatch(
                            param=param,
                            loaded_weight=loaded_weight,
                            weight_name=weight_name,
                            shard_id=shard_id,
