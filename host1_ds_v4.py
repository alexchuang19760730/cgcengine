...
class DeepseekV4Model(nn.Module):
    ...
    def forward(
        ...
        if layer_resume_mode and hidden_states_ref is not None:
            ...
            from sglang.srt.pipeline_parallel import load_hidden_states_from_ref

            hidden_states = load_hidden_states_from_ref(
                hidden_states_ref, device=input_ids.device, dtype=self.dtype
            )
        ...
        if layer_resume_mode and partial_kv_ref is not None:
            from sglang.srt.pipeline_parallel import load_partial_kv_into_forward_batch
