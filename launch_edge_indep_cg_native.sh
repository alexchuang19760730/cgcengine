#!/bin/bash
# Edge INDEPENDENT decode + cuda-graph ON, CGC kernels OFF (native sglang).
# Diagnose garbled output: if output is correct with CGC off, the garble comes
# from CGC kernel incompatibility (Host1 sgl-kernel vs Host2) after the full
# sglang sync. cuda-graph stays ON (proven to work).
set -u
