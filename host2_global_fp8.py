...
def _maybe_get_cpu_staging_alloc_kwargs() -> Dict[str, Any]:
    import os

    from sglang.srt.server_args import get_global_server_args

    server_args = get_global_server_args()
    if server_args is None or getattr(server_args, "cpu_offload_gb", 0) <= 0:
        return {}

    alloc_kwargs: Dict[str, Any] = {"device": "cpu"}
    disable_pin = str(os.environ.get("CGC_SGLANG_DISABLE_FP8_CPU_PIN_MEMORY") or "").strip().lower() in {"1", "true", "yes", "on"}
    if not disable_pin and is_pin_memory_available():
...
class Fp8MoEMethod(FusedMoEMethodBase):
    ...
    def __init__(self, quant_config: Fp8Config):
        ...
        logger.info(
            "Fp8MoEMethod init: quant_config_class=%s quant_name=%s "
            "is_fp4_experts=%s use_mxfp8=%s weight_block_size=%s "
            "is_checkpoint_fp8_serialized=%s",
            self.quant_config.__class__.__name__,
    ...
    def create_weights(
        ...
        **extra_weight_attrs,
        ...
        stage_large_weights_on_cpu = (
            self.block_quant
            and self.quant_config.is_checkpoint_fp8_serialized
            and not self.is_fp4_expert
            and not (_is_hip and _use_hip_int4)
        )
        moe_weight_alloc_kwargs = (
            _maybe_get_cpu_staging_alloc_kwargs() if stage_large_weights_on_cpu else {}
        )
