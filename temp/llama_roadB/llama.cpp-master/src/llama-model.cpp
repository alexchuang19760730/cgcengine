...
llama_model::~llama_model() {
    llama_expert_cache_free(expert_cache);
    for (auto * lora : loras) {
        delete lora;
    }
}
...
bool llama_model_base::load_tensors(llama_model_loader & ml) {
    ...
    // CGC expert-cache: copy index from loader to model
    if (ml.expert_cache_bytes > 0 && !ml.expert_index.empty()) {
        expert_index = std::move(ml.expert_index);
        fprintf(stderr, "llama_model: expert_index built: %zu entries\n", expert_index.size());
    }
...
size_t llama_model_expert_index_size(const llama_model * model) {
    return model ? model->expert_index.size() : 0;
}
