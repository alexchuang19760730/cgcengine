...
### 1. 產生第一份上報檔 `endpoints/windows-rtx4090.json`
...
### 2. 上報的 `platform` 寫對（Windows 是 `win32`，不是 `windows`）
...
### 4. 真的建過一次（CUDA，或退回 Vulkan），把結果寫進 `what_ran`
...
### 5. 回報 GPU 型號與建置設定（`gpu` / `build_profile`）
