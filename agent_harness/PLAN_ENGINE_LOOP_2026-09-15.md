...
| ↳ E1 更正（2026-09-17，實測） | 上面那格說「只結清一半」，理由是 `.venv` 不存在、`terminal-bench` 未安裝、`import` 止於 `No module named 'terminal_bench'`。**那個結論是用系統 python3 量出來的**：`tb_loop/.venv` 已經存在（`config.env` 的 `TB_VENV_PY` 指的就是它），`terminal-bench` 裝在裡面，用 `PYTHONPATH=agent_harness agent_harness/tb_loop/.venv/bin/python` 實測 **import OK** ⇒ 第一條驗收是綠的。同一輪掃到兩處搬家殘留並修好：`tb_loop/scripts/local_rehearsal.py:48` 的 `from agent_harness.agents.…`（連 `--help` 都跑不起來，而它第 42 行的註解早就改成 `tb_loop.…`）與 `agents/loopmoe_agent_adapter.py` 的 docstring 用法範例。全掃 `tb_loop`：0 處殘留、135 支 `.py` 可編譯、有 `--help` 的 entry 全部 rc=0。剩下的第 3 條（`tb run` smoke）缺的是 **Docker daemon**（colima 已停），不是模型端點 |
| ↳ E1 結清（2026-09-17 15:22，實跑） | 第 3 條 smoke **通過**：`PYTHONPATH=agent_harness agent_harness/tb_loop/.venv/bin/tb run -d "terminal-bench-core==0.1.1" --n-tasks 1` → **Resolved 1／Unresolved 0／Accuracy 100.00%**（`raman-fitting.easy`）。★ 上一格說「缺的是 Docker daemon」**只對了第一層**：起 colima 之後仍連續失敗兩次，真正的原因是被移除的 **Docker Desktop 留下的設定** —— ① `~/.docker/cli-plugins/` 底下 16 個連結全斷（都指向已不存在的 `/Applications/Docker.app`），docker CLI 靜默忽略它們，於是 `docker compose` 變成 `unknown command`；② `~/.docker/config.json` 的 `credsStore: "desktop"` 指向同一個已消失的 app，於是任何 registry 操作都報 `docker-credential-desktop not found`（而 `auths` 本來就是空的 ⇒ 移除該鍵不損失憑證）。處置：`brew install docker-compose`、修好 cli-plugin 連結、移除 `credsStore`（機器層，非 repo）。★ 兩件順帶：(1) `--agent` 的預設是 `oracle`（跑 gold solution）⇒ 這個 smoke **不需要模型**；(2) `tb_loop/README.md` 原本的入口 `python -m terminal_bench.cli.tb` 是壞的（該模組是 package，不能直接執行），已改成 venv 的 `tb`。對照表在 `agent_harness/README.md` 的更正 (c) |
...
| ↳ T1 的前置是三層，不是「等一個端點」（2026-09-17 17:0x，實測） | 本檔與 `README` 原本把 T1 寫成「`LLM_BASE_URL` 可換 ⇒ 語意上可指向遠端」。**那句是錯的**：`refine_engine.sh` 執行的是 `prime-agent -p --offline --model <REFINE_ENGINE_MODEL> --autonomous …`，模型由 **prime-agent 的設定**（`~/.prime/agent/models.json`）決定。實測三層：**① `prime-agent` CLI** 先前**不在機器上**（`~/.local/bin` 在 PATH 上但沒有它）——2026-09-17 17:05 以 `tb_loop/scripts/setup_env.sh` 第 2/3 步的官方安裝腳本裝好 0.9.5；**② provider** 只有 `turbofieldfare` → `http://localhost:8080/v1`、模型 `gemma-4-26b-a4b-it`、`api: openai-completions` ⇒ 裝好 CLI 也只會打本機 8080，遠端要在**prime-agent 的全域設定**（不在本 repo）加一條 provider；**③ `REFINE_ENGINE_MODEL`** 腳本刻意不給預設 | ⚠️ **仍未跑**。判準：`refine_engine.sh --dry-run` 在三層都不滿足時也是綠的（它只印 prompt：45,746 B）——**「dry-run 過了」不可以當成「可以跑」**。原先規劃的遠端出口是本機 CGC edge（`~/.local/bin/cgc`，:8000 OpenAI 相容／:4000 Anthropic 代理），但那個服務的 **API 半邊起不來**（`cgc_engine` 的 `sys.path` 指到已不存在的 `ComputeGraphCompiler-main`，實際目錄是 `CGC-main`）⇒ 已停回原狀；那是 `flashkv0516` 那條線的既有問題，本線不動它 |
...
| ↳ **E3 的 scaffold 已對真模型驗證**（2026-09-17 17:2x，兩次呼叫／~20 秒） | `closed_loop.build_prompt()` 的 docstring 原本寫「★ NOT YET VERIFIED AGAINST A REAL MODEL … `15+27` 必須答 `42`」——那是**一張可以用一次短請求關掉的欠條**，而它掛在那裡的理由一直是「唯一那顆 GPU 被另一條線的量測佔著」。實測（`scripts/run_server.sh`、Nail-Qwen3.6-MTP、`CGC_SERVER_CTX=40960`，prompt 位元組由 `build_prompt()` 自己產生、不手抄）：**正向 → `42`**（gen_tok=3、11.5 s）；**陰性**（同一個 prompt，只把生成段縮成裸的 `<|im_start|>assistant\n`）→ **進入 thinking 模式、把 64 token 額度用完而不作答**（gen_tok=64、8.2 s），**正是那個 scaffold 要防的失敗形態** | ✅ **scaffold 驗過**（`llm_client.py` 與 `build_prompt()` 的「未驗證」宣稱已改成有日期與證據的版本）。★ 沒有陰性對照的話，「42」只說明問題太簡單。★ 那兩個 timings 是**冷啟**（剛 map 完、機器在換頁）⇒ **不是性能數字，不得引用**。⚠️ 仍未跑的是**本體**：8 題 × 4 臂 × 3 reps ＝ 96 次呼叫，而它與 D6 欠帳是**同一次執行**（`compare.json` 一次吐出 `A_vs_B`／`B_vs_C`／`C_vs_D`）⇒ **一筆窗付兩筆欠帳，不是兩個窗**。★ **窗長要更正**：先前寫「prefill 12 次 × ~76 分鐘 ≈ 15 小時」，而那個 76 分鐘**不是量測值** —— `llm_client.py` 檔頭自己就寫明它是 6.74 t/s（一行只涵蓋前 `n_tokens = 400`、`progress = 0.01` 的進度行）× 30,594 token 的**外推**，而**同一段的下文**說當時 `vm.swapusage` 13,674/14,336 MB、Pages free 58 MB，並下了判語：「**那樣的狀態下產生的每個數字都是關於 swap 的數字，而不是關於模型的**」（與本 repo 的 `prefill250` 差 **37×**）。按 250 t/s 重算：12 次完整 prefill ≈ 28 分鐘，加 96 次短生成 ≈ **1 小時出頭**；前綴若完全沒被重用 ≈ 4.5 小時 —— 但那是**算術，不是量測**。⇒ 下一件事不是等窗，是跑 `--only Q1 --reps 1`（**4 次呼叫**）把真實 prefill 成本與快取行為釘住，再談那個窗要多大 |
...
| ↳ E4 的結清（2026-09-17；四項都到終態） | **① log 政策** 抽成資料 `shared/log_policy.json`（納管 6 條／不納管 5 條／上限／既有例外），由 `shared/check_log_policy.py` 機檢 —— 一啟動就抓到 `Backup/cgc_logs` 底下有 **12 個 `.log` 原文在版控裡**（5.6 MB、引用數 0），登記為可見例外。**② 拆分**：3310 行的 `scripts/check/knifeedge_matrix.py` → `scripts/check/knifeedge/` 11 個模組 ＋ 薄 shim（最大 571 行）；`shared/split_module.py` 逐位元組搬運，`shared/check_module_split.py` 五層證明（AST／runtime 指紋／CLI／替換契約／呼叫端），對照表 `shared/knifeedge_split_map.json`。**③ 分層** 以資料表達（`wrappers/classify.py` ＋ `classes.tsv`，46 支 6 類），不搬檔 —— `scripts/check/*` 有 562 條引用。**④ stale**：`shared/stale_registry.json` 單一權威；banner 跟著資料（`.replay_bench_baseline.json` 的 `_stale`）＋ 在消費點由 `wrappers/_common.sh` 的 `w_announce_stale` 印出 | ✅ 四項都可重跑：`check_log_policy.py --check --privacy`、`check_module_split.py --self-test`、`classify.py --check`、`check_stale.py --check`。⚠️ 兩項誠實邊界：**(a)** 拆分的「替換契約」在**路徑載入**（`spec_from_file_location`）下攔不到 `__setattr__`，那個情境要用 `km.<module>.<name> = x` —— 兩支離線自測已據此各改 5 處與 1 處，改前後輸出逐字元相同；**(b)** 那 12 個 `.log` 未移出（不是這條線的檔案），只有登記 |
