    ...
    void set_logits(struct llama_context * ctx, int idx) {
        ...
        if (sampled_probs) {
            ...
            if (getenv("CGC_MTP_DBG")) {
                fprintf(stderr, "MTPDBG set_logits: ctx=%p idx=%d path=sampled_probs count=%u n_vocab=%d first=[id=%d p=%.4f logit=%.4f]\n",
                        (void*)ctx, idx, sampled_probs_count, n_vocab, (int)sampled_ids[0], sampled_probs[0], sampled_logits[0]);
                fflush(stderr);
            }
            ...
            if (getenv("CGC_MTP_DBG")) {
                fprintf(stderr, "MTPDBG set_logits: ctx=%p idx=%d path=sampled_logits count=%u n_vocab=%d first=[id=%d logit=%.4f]\n",
                        (void*)ctx, idx, sampled_logits_count, n_vocab, (int)sampled_ids[0], sampled_logits[0]);
                fflush(stderr);
            }
        } else {
            ...
            if (getenv("CGC_MTP_DBG")) {
                fprintf(stderr, "MTPDBG set_logits: ctx=%p idx=%d path=default n_vocab=%d first=[%.4f %.4f %.4f]\n",
                        (void*)ctx, idx, n_vocab, logits[0], logits[1], logits[2]);
                fflush(stderr);
            }
        }
...
// [CGC §8.108] L4-pool variant: same as above but reads logits from external_logits
...
std::vector<llama_token> common_sampler_sample_and_accept_n(struct common_sampler * gsmpl, struct llama_context * ctx, const std::vector<int> & idxs, const llama_tokens & draft, const std::vector<const float *> & external_logits, const std::vector<common_draft_dist> * draft_dist, bool grammar_first) {
    ...
    // local helper mirroring common_sampler_sample, but bypassing set_logits(ctx, idx)
    auto sample_external = [&](const float * logits, bool apply_grammar_first) -> llama_token {
        GGML_ASSERT(logits != nullptr);
        ...
        // NOTE: backend sampling (llama_get_sampled_token_ith) is not supported here
        // because external_logits bypasses the ctx's output buffer. The CPU sampler
        // chain below always runs.
        auto & grmr    = gsmpl->grmr;
        auto & rbudget = gsmpl->rbudget;
        auto & chain   = gsmpl->chain;

        llama_sampler_apply(rbudget, &gsmpl->cur_p);

        if (apply_grammar_first && grmr) {
            llama_sampler_apply(grmr, &gsmpl->cur_p);
        }

        llama_sampler_apply(chain, &gsmpl->cur_p);
