...
// Batch all experts' down projection + weighted sum into one kernel
struct ggml_tensor * ggml_mul_mat_id_down_combine(
        struct ggml_context * ctx,
        struct ggml_tensor  * as,
        struct ggml_tensor  * b,
        struct ggml_tensor  * ids,
    ...
    GGML_ASSERT(b->ne[3] == 1);  // b is 3d
    GGML_ASSERT(ids->ne[2] == 1 && ids->ne[3] == 1); // ids is 2d
    GGML_ASSERT(weights->ne[2] == 1 && weights->ne[3] == 1); // weights is 2d
    GGML_ASSERT(ids->ne[1] == b->ne[2]); // must have an expert list per b row
    GGML_ASSERT(as->ne[0] == b->ne[0]); // can_mul_mat
    GGML_ASSERT(ids->ne[0] % b->ne[1] == 0); // can broadcast
    GGML_ASSERT(weights->ne[0] == ids->ne[0] && weights->ne[1] == ids->ne[1]);

    // output: [n_embd, n_tokens]
    const int64_t ne[4] = { as->ne[1], b->ne[2], 1, 1 };
    struct ggml_tensor * result = ggml_new_tensor(ctx, GGML_TYPE_F32, 4, ne);

    result->op     = GGML_OP_MUL_MAT_ID_DOWN_COMBINE;
    result->src[0] = as;
    result->src[1] = b;
    result->src[2] = ids;
