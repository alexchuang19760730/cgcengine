...
- **★ 方法論兩條（本檔要一起承擔）**：
  ...
  **自己造出相關**（18:45 那次複查裡 inactive 曾出現 r=+0.628、swap=−0.505，兩者都是假的）。
  ② **相關性報告必須附 `--json-glob`**：那個「n=8」的結果**我復現不出來** —— 我用
  `bisect_*`（n=6）、`prod_matrix/*`（n=10 篩 / n=24 不篩）、`phase_decomp/*`（n=15）三組都跑了，
  **方向一致但樣本數不同**。⇒ 一個相關性結論如果不寫它餵了哪些檔，就不是可重現的結論。
