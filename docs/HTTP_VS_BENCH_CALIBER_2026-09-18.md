...
> ### ⚠ 2026-09-18 18:45 更正（本文件 §7 的兩條「未查」已被工具回答）—— 上面那句**要打折**
>
> `scripts/check/caliber_env.py --equiv` 對 **prod25**（我實際跑的 profile）的裁決是
> **NOT CONFIG-EQUIVALENT**：
>
> | 旋鈕 | run_server.sh | bench cell | 裁決 |
> |---|---|---|---|
> | `-b / batch` | `<absent → default>` | **512** | **MISMATCH** |
> | `-ub / ubatch` | `<absent → default>` | **512** | **MISMATCH** |
> | （prefill250 下同一格） | **5632** | 512 | MISMATCH（**差 11×**） |
>
> 根因是**一行**：`scripts/check/prod_matrix.py:319` 的 `b = ub = spec["batch"]`（cell argparse）
> **先跑**，於是把 profile 經 `resolve()` 算出來的 BATCH/UBATCH 蓋掉。
> ⇒ **「唯一差別就是測量路徑」這句不成立**；那 +21%～+30% 目前只能標記為
> **「量測路徑 × batch 」（合併效應）**。在 `-b/-ub` 對齊（或加一個繼承 profile 的 cell）之前，
> **不能說它是口徑差、更不能拿去換算任何數字**。
>
> 同時修正一格**理由**（結論不變、論證方式變）：兩側的模型檔不是「同一個路徑」，
> 而是**同一份 bytes** —— size `13663116512` ＋ 5 個取樣視窗的 shasum **全同**（不同 inode）
> （`caliber_env.py --model-identity`）⇒ **`CGC_SERVER_MTP=0` 換出去的其實是同一份檔**。
> 這也回溯更正本 repo 記錄裡「`prod25-stream-mtpoff` 載的是第三個檔 ⇒ 不是同一受試對象」那句。
> **但它不是「沒差」**：MTP=0 會讓 **8 個 engine env 整塊消失**（`CGC_MM_BITIDENT`、
> `CGC_DRAFT_DECODE`、`CGC_VERIFY_DECODE`、`CGC_NO_PREFETCH`、layer caps…）
...
> ⚠ 18:45 補記：上一段的前提是「兩條路只差在量測路徑」。`--equiv` 顯示**不是**（§開頭更正）
> ⇒ 這段要等 `-b/-ub` 對齊後才成立；在那之前它只是**假設**。

## 7. 沒做的（具名）

- **prefill 的服務路徑數字**：沒有（§5①，工具限制）。
- **MTP on 的服務側數字**：沒有。本輪為了隔離口徑，兩側刻意都 MTP off。
- ~~**`-b 512` 與 server 自身 batch 設定是否等價**：未查~~ ⇒ **已答（18:45）：不等價。**
  `caliber_env.py --equiv` 對 prod25 判 **NOT CONFIG-EQUIVALENT**（server 側 `-b/-ub` **沒有給**、
  bench cell 硬編碼 512；prefill250 下則是 5632 vs 512，差 11×），根因 `prod_matrix.py:319`
  的 `b = ub = spec["batch"]`。⇒ 本檔的 +21~30% 降級為「路徑 × batch」的合併效應。
- ~~**環境的哪一項造成 bench 掉 17%**：未定位（swap 91% 是嫌疑，未證）~~ ⇒ **嫌疑人已排除兩個：**
  ① **記憶體不是**：decode **單 cell** 內各記憶體指標與吞吐的相關**全部低於臨界**
  （`--memory --cell-filter decode`；我用自己的三組 glob 重跑，n=6 臨界 0.807、n=10 臨界 0.631，
  兩種 n 下 |r| 全數未過）。
  ② **熱態不是**：依臂內 `worst` 分組，HEAVY 組內 spread **1.30×**、NOMINAL **1.39×**，
  而全體也只有 **1.54×** ⇒ 組內 ≈ 組間（此組數字來自 18:45 那次複查，我未重跑）。
  ⇒ **記憶體、熱態、引擎版本三個已記錄變數都排不出順序** ⇒ 這正是要走**配對設計**
  （抵銷未知慢漂，不必先知道它是誰）而不是繼續做後設相關的論據。
- **★ 方法論兩條（本檔要一起承擔）**：
  ① `--cell-filter` **是硬要求**（定義見 §8）：不篩 cell 會把 prefill（292/227）與 decode（7–11）
  放在同一條軸上，**自己造出相關**。
  ② **相關性報告必須附 `--json-glob`**：那個「n=8」我先是**復現不出來**（我用 `bisect_*` n=6、
  `prod_matrix/*` n=10/n=24、`phase_decomp/*` n=15 三組都跑了，方向一致但樣本數不同）；
  直到讀到 `docs/CALIBER_ENV_2026-09-18.md` 才看到它餵的是
  **`bisect_*` ＋ `duo_*` 兩個 glob 的聯集**，照著跑就**逐位複現**了（§8.4）。
  ⇒ 結論不變、而且更有力：**沒寫餵了哪些檔的相關性報告，是「當下不可重現」，不是「細節省略」。**
- **D5**：本輪改動 0 個 `src/`（只有 `docs/`）⇒ oracle 那一半不適用。

---

## 8. 工具規約：`--cell-filter` 為什麼是「硬要求」，以及「同一個 cell」怎麼定義

（本節回答 18:5x 的提問。`docs/CALIBER_ENV_2026-09-18.md` §5 已寫「`--cell-filter` 必用」——
那是**主張**；本節把它變成**可機檢的定義**，兩處衝突時以本節為準。）

### 8.1 「同一個 cell」＝「y 軸上那個數字是同一個量」的等價類

兩列可以進同一個統計 ⟺ 它們在下面這個 tuple 上逐項相等：

```
cell_key = (profile,
            metric,        # ★ prompt_per_second ｜ predicted_per_second ｜ …
            n_prompt, n_gen, depth,
            n_batch, n_ubatch,
            spec_state,    # draft-mtp n_max=3 ｜ none（★ 一整組旋鈕，不是一個旗標）
            pool_bytes)
```

判準是**量綱**，不是白名單：**y 軸的標籤必須每一列都一樣**。
prefill（`prompt_per_second`，292/227）與 decode（`predicted_per_second`，7–11）是**兩個不同的量**
⇒ **永遠不可同池，與 n 多大無關**。

- ★ `metric` 這一項**現在不在** cell_key 裡（工具只按 cell 名篩）——補它最便宜，因為「混池」最常見的
  形狀就是混到 metric。
- ★ 為什麼要等價類而不是「同一個 json 檔」：`prod_matrix.cell_command()` 的 cell 就是這個等價類的
  實作，而它 `:319` 蓋掉 profile 的 batch ⇒ **同一個 label 底下可以有不同 quantity**
  （那個檔自己的註解就寫了這個失敗模式：*same label, different quantity*）。

### 8.2 三態：一個統計只能是這三態之一

| 態 | 條件 | 可以寫成什麼 |
|---|---|---|
| `decided` | cell_key 完整 ∧ n ≥ 3 ∧ \|r\| ≥ r_crit(n) | finding；仍須附 n、r_crit、glob |
| `suggestion` | cell_key 完整 ∧ n ≥ 3 ∧ \|r\| < r_crit(n) | 只能寫「值得一次受控實驗」 |
| `cannot decide` | cell_key 不完整 ∨ n < 3 ∨ 沒有 arm 同時帶吞吐與環境讀數 | **要出聲的結果，不是失敗** |

第三態沿用 repo 既有語意（「不知道」不是紅燈，但**必須說出來**）。

### 8.3 四條讓它「硬」的可機檢條件

- **H1 預設拒絕**：沒給 cell_key ⇒ **不得印出任何相關係數**。（今天它會印，只在標頭寫 `cell=ALL`。）
- **H2 池化要明示**：跨 cell 只在顯式 `--allow-pooled` 之下才印，且每個數字要蓋 `INADMISSIBLE`。
- **H3 裁決要機器可讀**：這支工具**沒有 `--json`**（`--help` 的選項就那 9 個）⇒ 下游只能解析散文。
  必須輸出 `{cell_key, metric, n, r, r_crit, verdict}`。**一句 caveat 不是閘門。**
- **H4 突變式自測**（唯一能證明它會咬人的東西）：餵一份**故意混池**的 fixture（292/227 ＋ 7–11），
  斷言 ① 無 filter ⇒ rc≠0 且 `verdict ≠ decided`；② 有 filter ⇒ 同一批資料回到小 r。
  **沒有這一格，這條規則就只是註解**（本 repo 已判：可被跳過的閘門不是閘門）。

### 8.4 ★ 為什麼危險不是「它過了」，而是「它被抬到臨界線邊上」

18:56 用他們記下的兩個 glob 聯集（`bisect_*` ＋ `duo_*`）**逐位複現**：

| 指標 | 限單 cell（n=8，臨界 **0.705**） | 混池（n=10，臨界 **0.631**） | 抬升 |
|---|---|---|---|
| `inactive_gib` | **+0.182** | **+0.628** | **3.4×** |
| `anon_gib` | +0.315 | +0.596 | 1.9× |
| `swap_used_mib` | −0.256 | −0.505 | 2.0× |
| `free_gib` | −0.258 | −0.525 | 2.0× |

⇒ 混池把 |r| **抬了 2–3.4 倍**。而 `inactive` 那筆 **0.628 距臨界 0.631 只差 0.003** ——
**今天沒過線是運氣，不是安全**。
⇒ **這條規則的正當性不可以依賴「當下有沒有過線」**，否則規則會隨資料漂移：多一個 arm、
或讀者拿錯臨界（0.705 vs 0.631 就是兩個相鄰的 n），同一個假訊號就過了。

### 8.5 待辦（`caliber_env.py` 是別條線的未提交新檔 ⇒ 本節只定義、不動它）

四點最小修法：① H1 —— `if not args.cell_filter and args.memory: … return 2`；
② 把 `metric` 加進 cell_key；③ 加 `--json`（四態）；④ 加 H4 那一格自測。
**要我接手就說一聲**——那時它從「別人的未提交檔」變成「有 owner 的閘門」，我才動它。
