...
> 長期目標：**prefill 250+ / decode 25 t/s** ｜ 真實錨點：12.57–13.99 t/s（環境變動）｜ 本週末乾淨基線：10.73 t/s
...
| 成果 | 機制 | 實測 |
|---|---|---|
| **P0（skip-readraw）** | expert 權重不再全量讀進 CPU RAM，只從 pool pread | **swap ↓92%**；同環境 decode **+3.7%**、prefill **+29%** |
| **B 真臂（SPAC_HOT）** | 踢「累計路由次數」最低者（永不衰減），熱門重尾常駐 | hit 96.3%；HTTP ABBA **+4.3%**、llama-bench **+3.7%** |
...
改進 expert cache 容量（LAYER_CAPS / PIN_PROFILE），實測 **PIN_PROFILE −23%**（判死）——踢錯專家的代價遠大於省下的容量。
...
- prebind 失敗的根因（draft_prefetch_ids 在 MTP 架構下從未被填）——**讀代碼就知道，不需要跑實驗**
...
| **窗口守門（window_gate）** | 別的 llama-server 在跑就拒跑（乾淨窗口是等出來的，不是排出來的） |
...
| PIN_PROFILE（固定釘選） | −23%（踢錯專家代價 > 省容量） |
...
| D3 | PIN_PROFILE | **判死**：−23% |
| D4 | P0 skip-readraw | **成功**：swap 92%↓，零代價（同環境 +3.7%） |
| D5 | B 真臂 SPAC_HOT | **成功**：+4.3%/+3.7%，hit 96.3% |
...
| commit | 內容 |
|---|---|
| `41536c9e2` | B 真臂 SPAC_HOT + ABBA doc（+4.3% 標題帶成績） |
...
| `docs/REPLACEMENT_POLICY_75PCT_2026-09-23.md` | 路由重尾 98.8% / PIN_PROFILE 89.4% 實測 |
...
python3 scripts/check/llama_bench_matrix.py \
  --arms "prod-new:CGC_EXPERT_SKIP_READRAW=1;CGC_SPAC_HOT=1" \
  --prompt 2048 --gen 128 --depths 512 --reps 3 --ctx-size 0 \
  --workdir /tmp/commit_bench_p0hot --json /tmp/commit_bench_p0hot/result.json

# ABBA 三臂（重開機後）
RUN_GROUP=all PAIRS=3 COOL_S=300 bash scripts/check/abba_p0_3arm.sh
```
