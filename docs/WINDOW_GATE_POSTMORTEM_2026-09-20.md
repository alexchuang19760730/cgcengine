...
回收（停止讀之後，不再有任何操作）：20 s → 4470、40 s → 4575、60 s → 4736、80 s → 4775、
100 s → **4785 平台**（baseline 5456）。檔案頁被逐出（`ext` 3734 → 2400）、壓縮頁被解回
（`comp` 6471 → 4162）。
⇒ **大部分可逆（−2159 → −671 MB），但不是全部**：殘留的 ~670 MB 對應的是 `ext` 仍高 ~350 MB、
`comp` 仍高 ~1300 MB。**這是本探針自己造成的代價，照實記** —— 它會隨 cache 老化繼續衰減，
但它意味著「做一次大讀取」對等待中的那一條線**不是零成本**。
