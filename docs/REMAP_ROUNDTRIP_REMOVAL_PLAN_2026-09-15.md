...
#### 9.18.7 這個定位的後果：S1 是**探針**，不是缺陷

§9.18.4 的排除法不只減少了一個嫌疑，它同時改變了四件事：

**（a）D3 的第一個閘門活了下來——這是本節最重要的作用。** D3 的合法性建立在一句宣稱上：
「把查表搬到 GPU 不改數值」。若讀數是「ids 不同」⇒ GPU 上 `get_rows(table, selected_experts)`
**本身**就算錯 ⇒ D3 要動的那段圖重寫是錯的，今天的結論會是「D3 重新設計」。實際讀數是
39 層 × 117 node × 連續 3 個 graph 逐位元相同 ⇒ **D3 的候選實作在 bit 層面是對的**，
S1 過不了閘門的原因**不在 D3 要改的程式碼裡**。差別是「重新設計」vs「修一個既存缺陷」。

**（b）S1 的身分要改寫。** 在「輸入相同 + ids 相同 ⇒ 輸出不同」下，S1 沒有做錯任何事：
它只是**第一次讓一個既存的池內容缺陷變得可見**。基準臂之所以正確，是因為 host leaf 路徑
**by construction** 把映射寫死；S1 讓 GPU 真的去讀表、真的去讀池，於是讀到了池子**真實的樣子**。
⇒ **要修的不是 S1，是 residency。** 推論是普遍性的：**任何**把查表搬到 GPU 的方案（含 D3）
都會踩到同一個缺陷 ⇒ 這份定位不是 S1 的挫折，是 D3 的**先行風險清單被證實**。

**（c）三個異常收斂到一個機制候選（假說，待 §9.18.6 證實）。**

| 異常 | 出處 |
|---|---|
| S1 與基準分歧 | §9.18.3 |
| 分歧 **dispatcher 相依** | §9.13：分段不穩定（3 輪 3 值）、非分段穩定 |
| 閘門**失去判別力** | `eng-gate-0007`：不穩定 ⇒ 可能安靜假通過 |

若載體是**池內容**，而池由 host 在 step 內某時刻 `publish_slot_table` 發布 ⇒
「GPU 消費時刻 vs host 發布時刻」的相對順序決定讀到什麼；而**分段改變的正是這個順序**
（40 個 split = 更多 sync 點、更多 host 介入機會）。**一個機制同時解釋三格。**
標為**假說**：它給出的預測是「不穩定不該是隨機，而該隨發布時序而變」。

**（d）§9.17(f) 風險清單第 1 條由「前提」升格為「主線」。** 「`publish_slot_table` 發布是否
在熱路徑上」原本只是 D3 的一個前提風險；現在它與 §9.18.6 的唯一決定性測量**是同一問題的兩面**
——後者證「池內容是載體」，前者答「發布在不在熱路徑」。兩者合流：D3 的推論會不會塌，
取決於這一個讀數。

**（e）方法論：讀數的「時刻」是儀器的一部分（`eng-mh-0008`）。** 同一筆執行、同一塊 buffer，
主機探針說 `id_oob=16/16`（垃圾）、內核探針說合法；**兩者都對**，量的是不同時刻。
若沒有內核讀數，`id_oob=16/16` 會被當成「ids 算錯」的鐵證，整個調查會撲向一個**不存在的**
kernel bug。⇒ 這個定位之所以存在，是因為先做了一個**不依賴任何主機推論**的讀數。

**（f）量測範圍（依「S1 的所有結論都要標註 dispatcher」）。** 本節所有讀數都在**分段
dispatcher**（`OA_ASYNC=1`，全專案預設）下取得，且 capture `stride=8` ⇒ 「第一個 DIFF 是
layer 2」是關於 **token 0** 的。這兩條限制不影響 (a)(b)：ids 逐位元相同是**張量級**事實，
不是時序讀數，因此是現有 S1 讀數中**可移植性最高**的一個。
