#!/bin/bash
# Host1: sglang + Gemma 4 26B-A4B + MTP (steps=5, topk=2) + R-SWA monkey-patch
# R-SWA: caps full-attention decode to reference(4) + window(128) = 132 tokens
# Uses monkey-patch loader (no sglang source modification)
...
# Monkey-patch loader path
export PYTHONPATH=/root/flashkv0516/rswaengine/python:/root/flashkv0516:$PYTHONPATH

cd /root/flashkv0516

# Import patch loader first, then launch sglang
exec /data/venv_gemma4/bin/python3 -c "
import rswa_patch_loader
import sglang
